...
class SiameseNetworkDataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        img_feature = resnet(img_pile.unsqueeze(0))
        mask_feature = resnet(img_mask.unsqueeze(0))
        features = torch.cat((img_feature, mask_feature),1)

        return features, torch.from_numpy(np.array([int(label_)], dtype=np.float32))
...
for epoch in range(50):
    ...
    with torch.no_grad():
        ...
        for feature, label in val_dataloader:
            ...
            correct_t += torch.sum(pred_==label).item()

...
for i in range(num_files):
    ...
    rnd_pile = np.char.replace(rnd_pile, 'pile_features', 'pile_imgs')
    rnd_pile = np.char.replace(rnd_pile, 'npy', 'jpg')
    masked_pile_images = Image.open(str(rnd_pile))  

    rnd_mask = np.char.replace(rnd_mask, 'mask_features', 'mask_imgs')
    rnd_mask = np.char.replace(rnd_mask, 'npy', 'jpg')
    masked_image = Image.open(str(rnd_mask)) 

    w_, h_ = masked_image.size
